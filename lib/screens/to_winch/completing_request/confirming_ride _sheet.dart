...
class _RideBottomSheetState extends State<RideBottomSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer3<WinchRequestProvider, MapsProvider,CustomerCarProvider>(
      builder: (context, WinchRequestProvider, MapsProvider, CustomerCarProvider, child) =>
          DraggableScrollableSheet(
            ...
            builder: (BuildContext myContext, myController) {
              ...
              ConfirmWinchServiceRequestModel winchRequestModel = new ConfirmWinchServiceRequestModel(
                ...
                estimatedDistance: MapsProvider.tripDirectionDetails.distanceText,
                    ...
                    child: Column(
                      children: [
                        ...
                        Padding(
                          ...
                          child: RaisedButton(
                            onPressed: () async {
                              await WinchRequestProvider.confirmWinchService(winchRequestModel, jwtToken);
                              if (WinchRequestProvider.isLoading == false && WinchRequestProvider.STATUS_SEARCHING == true)
                                  {
                                    Navigator.of(myContext).pop();
                                    searchForNearestWinchSheet(context);
                                    print(WinchRequestProvider.confirmWinchServiceResponseModel.error);
                                    print(WinchRequestProvider.confirmWinchServiceResponseModel.status);
                                    timer = Timer.periodic(Duration(seconds: 14), (timer) async
                                      {
                                        await WinchRequestProvider.checkStatusForConfirmedWinchService(jwtToken);
                                        if (WinchRequestProvider.STATUS_TERMINATED == true)
                                            {
                                               print("process terminated");
                                              timer.cancel();
                                              //searchForNearestWinchSheet(context);
                                            }
                                        else if (WinchRequestProvider.STATUS_ACCEPTED == true)
                                            {
                                              print("client found");
